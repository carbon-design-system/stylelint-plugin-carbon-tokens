...
export const messages = utils.ruleMessages(ruleName, {
  rejected: (property, value) =>
    `Expected carbon theme token, mixin or function for "${property}" found "${value}."`,
  rejectedUndefinedRange: (property, value, range) =>
    `Expected carbon theme token, mixin or function for "${property}" found "${value} in position(s) "${range}"."`,
  rejectedVariable: (property, variable, value) =>
    `Expected carbon theme token, mixin or function to be set for variable "${variable}" used by "${property}" found "${value}".`,
});
