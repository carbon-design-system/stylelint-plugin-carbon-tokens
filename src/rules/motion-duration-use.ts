...
import type { MotionDurationRuleOptions } from '../types/index.js';
...
const ruleFunction: stylelint.Rule<boolean, MotionDurationRuleOptions> = (
  primaryOption,
  secondaryOptions,
  context
) => {
  return async (root, result) => {
    ...
    const options: MotionDurationRuleOptions = {
      ...defaultOptions,
      ...secondaryOptions,
    };

    // Load Carbon motion tokens and extract duration tokens
    const motionTokens = loadMotionTokens();
    const tokens = motionTokens.duration;
