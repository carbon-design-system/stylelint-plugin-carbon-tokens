...
var generatedTests = function generatedTests() {
  ...
  var reject = [];
  var props = ["margin"];
  var good = ["$spacing-01", "$layout-01", "-$spacing-01", "-$layout-01", "$carbon--spacing-01", "$carbon--layout-01", "100%", "50%", "100vw", "50vw", "100vh", "50vh", "0xxx", "$my-value-accept", "var(--my-value-accept)"];
  var bad = ["$container-01", // only bad for default options
  "$fluid-spacing-01", "$icon-size-01", "$carbon--container-01", "$carbon--fluid-spacing-01", "$carbon--icon-size-01", "199px", "299px", "399px", "499px", "$my-value-reject", "var(--my-value-reject)"];
  ...
  for (var _i = 0, _props = props; _i < _props.length; _i++) {
    ...
    for (var g = 0; g < good.length - 4; g++) {
      ...
      for (var n = 1; n < 5; n++) {
        // number of values
        accept.push({
          code: ".foo { ".concat(prop, ": ").concat(good.slice(g, g + n), "; }"),
          description: "A ".concat(prop, " using ").concat(n, " token(s) is accepted")
        });
      }
    }
    ...
    for (var b = 0; b < bad.length - 4; b++) {
      ...
      for (var _n = 1; _n < 5; _n++) {
        // number of values
        reject.push({
          code: ".foo { ".concat(prop, ": ").concat(bad.slice(b, b + _n), "; }"),
          description: "A ".concat(prop, " using ").concat(_n, " non-token(s) is rejected")
        });
      }
    }
  }
