...
### Problem
The five rule files ([`theme-use`](src/rules/theme-use.ts:1), [`layout-use`](src/rules/layout-use.ts:1), [`type-use`](src/rules/type-use.ts:1), [`motion-duration-use`](src/rules/motion-duration-use.ts:1), [`motion-easing-use`](src/rules/motion-easing-use.ts:1)) contained ~80% duplicate boilerplate code:
...
### Solution
Created a rule factory function at [`src/utils/create-rule.ts`](src/utils/create-rule.ts:1) that encapsulates all common behavior.
...
5. **Optional shouldSkipValue** - For rule-specific value skipping (motion-easing)
...
**Total**: 693 lines → 216 lines + 143 lines (factory) = **359 lines** (48% reduction)
...
The factory pattern doesn't prevent this organization - it just eliminates boilerplate.
...
That's it! No need to reimplement validation, options handling, or auto-fix logic.
